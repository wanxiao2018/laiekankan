/*
 * Local QR renderer for 去俄看看.
 *
 * The QR encoder is based on Kazuhiko Arase's QR Code for JavaScript,
 * distributed under the MIT license. It is bundled here so QR creation
...
module.exports = QRCode;
  ...
  function load(name, from) {
    const id = resolve(name, from || '');
    if (cache[id]) return cache[id].exports;
    if (!modules[id]) throw new Error('Unknown QR module: ' + id);
    const module = { exports: {} };
    cache[id] = module;
    modules[id](dependency => load(dependency, id), module, module.exports);
    return module.exports;
  }
  const QRCode = load('index');
  const QRErrorCorrectLevel = load('QRErrorCorrectLevel');
  function draw(canvas, text, options, callback) {
    const settings = options || {};
    try {
      const qr = new QRCode(-1, QRErrorCorrectLevel[settings.errorCorrectionLevel || 'M'] || QRErrorCorrectLevel.M);
      qr.addData(String(text));
      qr.make();
      const size = Number(settings.width || canvas.width || 220);
      const margin = Math.max(0, Number(settings.margin ?? 2));
      const count = qr.getModuleCount();
      const scale = Math.floor(size / (count + margin * 2));
      if (!Number.isFinite(scale) || scale < 1) throw new Error('QR canvas is too small');
      const actual = count * scale;
